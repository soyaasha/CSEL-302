...
species student {
    ...
    reflex update_attention {
        ...
        // Attention change depending on break
        if (is_break) {
        ...
        // Performance improves when attention is high
        if (attention > 0.6) {
            performance <- min(1.0, performance + 0.01);
        }

        // -------- FATIGUE MODEL --------
        // Track consecutive low-attention cycles
        if (attention < 0.3) {
        ...
        // If low attention lasts 10 cycles, performance decreases
